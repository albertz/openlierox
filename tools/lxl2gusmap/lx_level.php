...
function LXLevelInfoAdv($level, $minimap_w = 128, $minimap_h = 96, $hq = false)
{
  ...
  switch ($returnValue->Type)  {
    case LXL_IMAGE:
      if(! LoadImageFormat($fp, $returnValue->Width, $returnValue->Height, $returnValue) )
      	return false;
	  break;
    default:
    	return false;
...
function LoadImageFormat($fp, $width, $height, $m)
{
  ...
  $m->frontImage = imagecreatetruecolor($width, $height);
  $m->backImage = imagecreatetruecolor($width, $height);
  $m->matImage = imagecreate($width, $height);
  
  // in this order to have rock=0, back=1, dirt=2 in palette
  $matRock = imagecolorallocate($m->matImage, 0,0,0);
  $matBack = imagecolorallocate($m->matImage, 255,255,255);
  $matDirt = imagecolorallocate($m->matImage, 255,0,0);
  
  $frontTransp = imagecolorallocat($m->frontImage, 255, 0, 255);
  ...
  for ($y = 0; $y < $height; $y++)  {
    for ($x = 0; $x < $width; $x++)  {
      ...
      if (ord($uncompressed[$mi]) & PX_EMPTY)  { // background
        $color = imagecolorallocate($m->backImage, ord($uncompressed[$bi]),
                 ord($uncompressed[$bi+1]), ord($uncompressed[$bi+2]));
        imagesetpixel($m->backImage, $x, $y, $color);
        imagesetpixel($m->frontImage, $x, $y, $frontTransp);
        imagesetpixel($m->matImage, $x, $y, $matBack);
      } else { // foreground
        $color = imagecolorallocate($m->frontImage, ord($uncompressed[$fi]),
                 ord($uncompressed[$fi+1]), ord($uncompressed[$fi+2]));
        imagesetpixel($m->frontImage, $x, $y, $color);
        
        if(ord($uncompressed[$mi]) & PX_ROCK)
	        imagesetpixel($m->matImage, $x, $y, $matRock);
	   	else
	        imagesetpixel($m->matImage, $x, $y, $matDirt);    	
      }
