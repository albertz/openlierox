...
---lightflare
local lightflare = load_particle("lightflare.obj")
local lightflare2 = load_particle("lightflare2.obj")
...
---Transportal
local portal = load_particle("portal.obj")
local teleport = load_particle("portal_use.obj")
local t_i = sounds["teleport_in.ogg"]	
local t_o = sounds["teleport_out.ogg"]
local teleport_effect = load_particle("portal_flash.obj")

---Shotgun
local shotgun_shell = load_particle("shotgun_shell.obj")
...
function doom.slimpack_speed_reset(worm)
	...
	if player:data().slimpack_speed then
		 ...
		 if player:data().slimpack_speed % 2 == 0 then
...
worm:player():data().charge_on = ((worm:player():data().grenade_hold * 100)/3.5)
...
worm:player():data().nocrosshair = nil
worm:player():data().nocrosshair_medium = nil
worm:player():data().nocrosshair_large = nil
end
...
if worm:player():data().ar_fire >= 0 and  worm:player():data().ar_fire < 3 then
worm:player():data().nocrosshair = nil
worm:player():data().nocrosshair_medium = nil
worm:player():data().nocrosshair_large = nil
end

if worm:player():data().ar_fire >= 3 and  worm:player():data().ar_fire < 6 then
worm:player():data().nocrosshair = 1
worm:player():data().nocrosshair_medium = 1
worm:player():data().nocrosshair_large = nil
end

if worm:player():data().ar_fire >= 6  then
worm:player():data().nocrosshair = 1
worm:player():data().nocrosshair_medium = nil
worm:player():data().nocrosshair_large = 1
end
...
worm:player():data().charge_on = ((worm:player():data().bfg_fire * 100)/255)
...
worm:shoot (smoke,1,0,0.02,0,0,10,0,6)
end
end
...
worm:player():data().charge_on = ((worm:player():data().ion_fire * 100)/120)
...
---------------------------------Transportal-------------------------
print("Loading Transportal. . .")

function doom.transportal_using(worm)
local player = worm:player()
if player then
worm:player():data().transportal_fire = 1
end
end

function doom.transportal_press(worm,player)
local player = worm:player()
if worm:player():data().transportal_fire then
worm:player():data().transportal_fire = worm:player():data().transportal_fire + 1
end
if worm:player():data().transportal_fire  == 3 then
worm:player():data().transportal_fire = 1
end
if worm:player():data().transportal_fire  == 2 then
worm:shoot(portal,1,1,0,0,0,0,0,4)
end
if worm:player():data().transportal_fire  == 1 then
worm:shoot(teleport_effect,1,0,0,0,0,0,0,0)
player:data().slimpack_speed = nil
player:data().slimpack_speed_sound = nil
tx2 = transportalX
ty2 = transportalY
worm:set_pos(tx2,ty2)
t_o:play(worm, nil, 100, 1, 0)
end
end

function doom.transportal_release(object,player)
	local tx,ty = object:pos()
		transportalX = tx
		transportalY = ty
	end

function doom.transportal_use(object)
	 if tx2 or ty2 then 
	 object:shoot(teleport,1,0,0,0,0,0,0,0)
	 object:shoot(teleport,1,0,0,0,0,0,0,0)
	 object:shoot(teleport_effect,1,0,0,0,0,0,0,0)
	 t_i:play(object, nil, 100, 1, 0)
	 tx2 = nil
	 ty2 = nil
end
end
...
function doom.homing_needler (object,worm)
	local player = object:player()
...
----------------------------needler----------------------------
-----------------------------------lightflare----------------------------
print("Loading Lightflare. . .")

function doom.lightflare_reset(worm)
worm:player():data().lightflare_hold = 1
worm:player():data().lightflare_reload = 2
end

function doom.lightflare_loop(worm)
worm:player():data().charge_on = ((worm:player():data().lightflare_hold * 100)/3.5)
if worm:player():data().lightflare_reload then
worm:shoot (lightflare2,1, 0 , 0 ,0 , 0, 0, 0, 0, 6)
end
if worm:player():data().lightflare_hold then
worm:player():data().lightflare_hold = worm:player():data().lightflare_hold + 0.15
end
if worm:player():data().lightflare_hold > 3.5 then
worm:player():data().lightflare_hold = worm:player():data().lightflare_hold - 0.15
end
end

function doom.lightflare_release(worm)
worm:player():data().charge_on = 0
if worm:player():data().lightflare_reload then
worm:shoot (lightflare,1, worm:player():data().lightflare_hold , 1 ,0.1 , 0.5, 0, 10, 0, 6)
worm:player():data().lightflare_hold = 1
end
end

function doom.lightflare_out_of_ammo(worm)
worm:player():data().lightflare_reload = nil
end

function doom.lightflare_reload(worm)
worm:player():data().lightflare_reload = 1
end
-----------------------------------lightflare----------------------------

-------------------------------Crosshair-stuff---------------------------
print("Loading Crosshairs. . .")
	function doom.crosshair_reset(worm)
		worm:player():data().charge_on = 0
		worm:player():data().nocrosshair = nil
		worm:player():data().nocrosshair_medium = nil
		worm:player():data().nocrosshair_large = nil
		worm:player():data().crosshair_plasmarifle = nil
		worm:player():data().crosshair_needler = nil
		worm:player():data().crosshair_bfg = nil
		worm:player():data().crosshair_flamethrower = nil
		worm:player():data().crosshair_ioncannon = nil
		worm:player():data().crosshair_railgun = nil
		worm:player():data().crosshair_chaingun = nil
end

	function doom.crosshair_set(worm)
		worm:player():data().nocrosshair = 1
end

	function doom.crosshair_set_plasmarifle(worm)
		worm:player():data().crosshair_plasmarifle = 1
		worm:player():data().nocrosshair = 1
end

	function doom.crosshair_set_needler(worm)
		worm:player():data().crosshair_needler = 1
		worm:player():data().nocrosshair = 1
end

	function doom.crosshair_set_bfg(worm)
		worm:player():data().crosshair_bfg = 1
		worm:player():data().nocrosshair = 1
end

	function doom.crosshair_set_flamethrower(worm)
		worm:player():data().crosshair_flamethrower = 1
		worm:player():data().nocrosshair = 1
end

	function doom.crosshair_set_ioncannon(worm)
		worm:player():data().crosshair_ioncannon = 1
		worm:player():data().nocrosshair = 1
end

	function doom.crosshair_set_chaingun(worm)
		worm:player():data().crosshair_chaingun = 1
		worm:player():data().nocrosshair = 1
end

	function doom.crosshair_set_railgun(worm)
		worm:player():data().crosshair_railgun = 1
		worm:player():data().nocrosshair = 1
end
-------------------------------Crosshair-stuff---------------------------
---------------------------------------Shotgun---------------------------
print("Loading Shotgun. . .")	
	function doom.shotgun_set(worm)
	worm:player():data().shotgun_shell = nil
	end

	function doom.shotgun_fire(worm)
	worm:player():data().shotgun_shell = 1
	end

	function doom.shotgun_loop(worm)
	if worm:player():data().shotgun_shell then
	worm:player():data().shotgun_shell = worm:player():data().shotgun_shell + 1
	end
	if worm:player():data().shotgun_shell == 80 then
	worm:player():data().shotgun_shell = nil
	worm:shoot(shotgun_shell, 1 , 1, 0.1, 0.2, 0, 10, -105)
	end
end
---------------------------------------Shotgun------------------------------
