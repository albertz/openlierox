...
int player_count=0;
int local_players=0;
Net_ClassID  player_classid = Net_Invalid_ID;
...
void send_msg()
{
  if(player[local_player[0]] && player[local_player[0]]->node)
  {
    ...
    if(player[local_player[0]]->node->getRole()==eNet_RoleAuthority)
    {
      char msg2[1024];
...
void worm::sendmsg(char *msg)
{
  if(node)
  {
    Net_BitStream *data=Net_Control::Net_createBitStream();;
    data->addInt(3,8);
    data->addString( msg );
    node->sendEvent(Net_Node::eEventMode_ReliableOrdered, Net_REPRULE_OWNER_2_AUTH | Net_REPRULE_AUTH_2_ALL, data);
  };
};
...
void change_nick(int pl)
{
  ...
  /*if(player[local_player[pl]] && player[local_player[pl]]->node)
  {
    ...
    player[local_player[pl]]->node->sendEvent(Net_Node::eEventMode_ReliableOrdered, Net_REPRULE_OWNER_2_AUTH, data);
  };*/
};
...
void worm::shooteventsend()
{
  ...
  node->sendEvent(Net_Node::eEventMode_ReliableOrdered, Net_REPRULE_AUTH_2_ALL, data);
  srand(t);
};
...
void worm::checkevents()
{
  ...
  {
    Net_Node::eEvent type;            // event type
    eNet_NodeRole    remote_role;     // role of remote sender
    Net_ConnID       conn_id;         // connection id of sender
    int event;
  
    Net_BitStream *data = node->getNextEvent(&type, &remote_role, &conn_id);
    if (type == Net_Node::eEvent_AuthorityRemoved)
		{
			con->log.create_msg("PLAYER REMOVED");
			deleteme = true;
		} else
    if (type == Net_Node::eEvent_User)
    {
      event=data->getInt(8);
      if(event==4)
      {
      ...
      }else if(event==3)
      {
        ...
        con->log.create_msg(msg2);
        con->echolist.add_echo(msg2);
        if (node->getRole()==eNet_RoleAuthority)
        {
          sendmsg(msg);
        };
        play_sample(game->menu_select->snd, *game->VOLUME, 127, 1300, 0);
...
worm::worm()
{
    ...
    //node->addInterpolationInt(&yspd,32,false,Net_REPFLAG_MOSTRECENT,Net_REPRULE_AUTH_2_ALL,99,1000,NULL,-1,-1,0);
    //node->addInterpolationInt(&xspd,32,false,Net_REPFLAG_MOSTRECENT,Net_REPRULE_AUTH_2_ALL,99,1000,NULL,-1,-1,0);
    node->addReplicationInt((zS32*)&ropestate,8,false,Net_REPFLAG_MOSTRECENT,Net_REPRULE_AUTH_2_ALL,99,-1,-1);
    node->addReplicationInt((zS32*)&ropex,32,false,Net_REPFLAG_MOSTRECENT,Net_REPRULE_AUTH_2_ALL,99,-1,-1);
    node->addReplicationInt((zS32*)&ropey,32,false,Net_REPFLAG_MOSTRECENT,Net_REPRULE_AUTH_2_ALL,99,-1,-1);
    node->addReplicationInt((zS32*)&ropexspd,32,false,Net_REPFLAG_MOSTRECENT,Net_REPRULE_AUTH_2_ALL,99,-1,-1);
    node->addReplicationInt((zS32*)&ropeyspd,32,false,Net_REPFLAG_MOSTRECENT,Net_REPRULE_AUTH_2_ALL,99,-1,-1);
    node->addReplicationInt((zS32*)&aim,32,false,Net_REPFLAG_MOSTRECENT,Net_REPRULE_OWNER_2_AUTH|Net_REPRULE_AUTH_2_PROXY,99,-1,-1);
    node->addReplicationInt((zS32*)&dir,8,true,Net_REPFLAG_MOSTRECENT,Net_REPRULE_AUTH_2_ALL,99,-1,-1);
    node->addReplicationBool(&keys->fire,Net_REPFLAG_RARELYCHANGED,Net_REPRULE_OWNER_2_AUTH,false,-1,-1 );
    node->addReplicationBool(&keys->left,Net_REPFLAG_RARELYCHANGED,Net_REPRULE_OWNER_2_AUTH,false,-1,-1 );
    node->addReplicationBool(&keys->right,Net_REPFLAG_RARELYCHANGED,Net_REPRULE_OWNER_2_AUTH,false,-1,-1 );
    node->addReplicationBool(&keys->up,Net_REPFLAG_RARELYCHANGED,Net_REPRULE_OWNER_2_AUTH,false,-1,-1 );
    node->addReplicationBool(&keys->down,Net_REPFLAG_RARELYCHANGED,Net_REPRULE_OWNER_2_AUTH,false,-1,-1 );
    node->addReplicationBool(&keys->jump,Net_REPFLAG_RARELYCHANGED,Net_REPRULE_OWNER_2_AUTH,false,-1,-1 );
    node->addReplicationBool(&keys->change,Net_REPFLAG_RARELYCHANGED,Net_REPRULE_OWNER_2_AUTH,false,-1,-1 );
    ...
    for(o=0;o<5;o++)
      node->addReplicationInt((zS32*)&weap[o].weap,32,false,Net_REPFLAG_RARELYCHANGED|Net_REPFLAG_MOSTRECENT,Net_REPRULE_OWNER_2_AUTH|Net_REPRULE_AUTH_2_PROXY,0,-1,-1 );
