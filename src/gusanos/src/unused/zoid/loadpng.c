 ...
 */
/* Modifications to allow C++ compiler to compiler properly - nym 2k4 */
...
static int check_if_png(PACKFILE *fp)
{
    ...
    return (png_sig_cmp((png_byte*)buf, (png_size_t)0, PNG_BYTES_TO_CHECK) == 0);
}
...
{
    return (png_sig_cmp((png_byte*)buffer, (png_size_t)0, PNG_BYTES_TO_CHECK) == 0);
}
...
BITMAP *load_memory_png(AL_CONST void *buffer, int bufsize, RGB *pal)
{
    ...
    memory_reader_state.buffer = (char*)buffer;
