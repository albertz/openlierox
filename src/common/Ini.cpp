...
// 11/14/2003 -- Use "__DONTINCLUDE_H__" instead of "_AFXDLL" to determine MFC presence
...
// may be truncated.
DWORD CIni::GetStr(LPCTSTR lpSection, LPCTSTR lpKey, LPTSTR lpBuffer, DWORD dwBufSize, LPCTSTR lpDefault) const
{
	if (lpBuffer != NULL)
		*lpBuffer = _T('\0');
...
#ifdef __DONTINCLUDE_H__
CString CIni::GetStr(LPCTSTR lpSection, LPCTSTR lpKey, LPCTSTR lpDefault) const
{
	LPTSTR psz = __GetStringDynamic(lpSection, lpKey, lpDefault);
	CString str(psz);
	delete [] psz;
...
int CIni::GetInt(LPCTSTR lpSection, LPCTSTR lpKey, int nDefault, int nBase) const
{
	...
	GetStr(lpSection, lpKey, sz, DEF_PROFILE_NUM_LEN);
