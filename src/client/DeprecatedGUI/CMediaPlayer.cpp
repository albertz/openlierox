...
/*	if (firstcall)
	...
	if(!include_subdirs)  {
		SongListFiller filler(this);
		FindFiles(filler, dir, false, FM_REG);
		return;
	}
	...
	while(DrawLoadingProgress() && cStack.Pop(current_dir)) {
		...
		SongListFiller filler(this);
		FindFiles(filler, current_dir, false, FM_REG);
		PlaylistLoader loader(this);
		FindFiles(loader, current_dir, false, FM_DIR);
	}
}
