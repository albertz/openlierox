...
///////////////////
// Load an image
SDL_Surface* _LoadImage(const UCString& filename)
{
	...
	UCString fname = GetFullFileName(filename);
	if(fname.size() == 0)
		return NULL;
...
SDL_Surface *CCache::LoadImgBPP(const UCString& _file, bool withalpha) {
	Type = CCH_IMAGE;
	Filename = _file;
...
///////////////////
// Load a sample
SoundSample* CCache::LoadSample(const UCString& _file, int maxplaying)
{
	Type = CCH_SOUND;
	Filename = _file;
	
	UCString fullfname = GetFullFileName(Filename);
	if(fullfname.size() == 0)
	{
