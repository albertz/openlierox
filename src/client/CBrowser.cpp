...
///////////////////
// Load the cmht file
int CBrowser::Load(const UCString& sFilename)
{
	FILE *fp;
...
void CBrowser::AddObject(const UCString& sText, const UCString& sVal, int iType, int iEnd)
{
	ht_object_t *obj;
	int r,g,b;
	...
	if(!iEnd) {
		switch(iType) {
			...
			case HTO_COLOUR:
			case HTO_BOX:
				const std::vector<UCString>& tok = explode(sVal,",");
				if(tok.size() >= 3) {
