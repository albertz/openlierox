...
int Menu_Net_LANInitialize(void)
{
	...
	cLan.Add( new CListview(),							   ServerList, 40, 180, 560, 240);
	cLan.Add( new CLabel("Select player:",0xffff),-1,		125, 152, 180,15);
	cLan.Add( new CCombobox(),								PlayerSelection,		225,150, 170,  19);
	...
	for(;p;p=p->tNext) {
		...
		cLan.SendMessage( PlayerSelection, CBM_ADDITEM, p->iID, (DWORD)p->sName);
		cLan.SendMessage( PlayerSelection, CBM_SETIMAGE, p->iID, (DWORD)p->bmpWorm);
	}

	cLan.SendMessage( PlayerSelection, CBM_SETCURINDEX, tLXOptions->tGameinfo.iLastSelectedPlayer, 0);
...
void Menu_Net_LANShutdown(void)
{
	...
	cb_item_t *item = (cb_item_t *)cLan.SendMessage(PlayerSelection,CBM_GETCURITEM,0,0);
	if (item)
			...
			case ServerList:
                ...
                if( ev->iEventMsg == LV_RIGHTCLK ) {
					...
					if(result && addr[0]) {
						...
						cLan.SendMessage( PopupMenu, MNM_ADDITEM, 3, (DWORD)"Add to favourites" );
						cLan.SendMessage( PopupMenu, MNM_ADDITEM, 4, (DWORD)"Send \"I want join message\"" );
                        cLan.SendMessage( PopupMenu, MNM_ADDITEM, 5, (DWORD)"Server details" );
                    }
                }
				break;
                    ...
                    case MNU_USER+4:
						{
							server_t *sv = Menu_SvrList_FindServerStr(szLanCurServer);
							char Nick[256];
							cLan.SendMessage(PlayerSelection, CBM_GETCURNAME, (DWORD)Nick, sizeof(Nick));
							Nick[255] = '\0'; // safety
							char *sNick = Nick;
							if (sv)
