...
typedef std::vector<tString> song_list;
...
CGuiLayout cOpenGui;
	...
	// Methods
	void		ReFillList(CListview *lv, const tString& dir);
	bool		IsRoot(const tString& dir);
public:
	tString Execute(const tString& default_dir);
...
public:
	void		Clear(void);
	void		Load(const tString& dir,bool include_subdirs, bool add_to_current_pl);
	tString	GetCurSong(void);
	...
	void		SaveToFile(const tString& filename,bool absolute_path);
	void		LoadFromFile(const tString& filename,bool absolute_path);
};
...
public:
	 ...
	 void		Shutdown(void);
	 tString	GetNameFromFile(const tString& path);
	 void		Play(void);
	 ...
	 inline void	OpenDirectory(const tString& dir,bool include_subdirs=true,bool add_to_current_pl=true)  {tPlayList.Load(dir,include_subdirs,add_to_current_pl); }
	 inline const tString& GetCurrentSongName(void)	{return szCurSongName; }

	 void			LoadPlaylistFromFile(const tString& filename,bool absolute_path=false);
	 inline void	SavePlaylistToFile(const tString& filename,bool absolute_path=false)  {tPlayList.SaveToFile(filename,absolute_path); }
...
public:
	CPlayerMarquee(const tString& text, Uint32 col);
...
public:
	...
	inline tString getText(void)	{ return szText; }
	inline void setText(const tString& text)	{ szText = text; iFrame = 0; fTime=0; RedrawBuffer(); }
