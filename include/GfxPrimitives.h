...
SDL_Surface*	LoadImage(const UCString& _filename, bool withalpha = false);
...
inline bool Load_Image(SDL_Surface*& bmp, const UCString& name)  {
	bmp = LoadImage(name); 
	if (bmp == NULL)  { 
...
inline bool Load_Image_WithAlpha(SDL_Surface*& bmp, const UCString& name)  {
...
bool	SaveSurface(SDL_Surface *Image, const UCString& FileName, int Format, bool Tournament);
